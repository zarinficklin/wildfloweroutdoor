...
class Hana_Templater{
	 ...
	 * @param $cat_id the category that corresponds to the current instance (each instance represents a different category)
	 * @param $custom_page a custom page object that represents the custom page
	 * @param $prefix the prefix of the fields in the form
	 */
	function get_custom_page_form_template($title, $cat_id, $custom_page, $prefix){
		...
		foreach($custom_page->fields as $field){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					$button_id='upload_button'.$cat_id;
					$field_id=$field['id'];
					//print a field with an upload button
					$html.= '<input class="option-input upload hana-upload '.$class.'" name="'.$prefix.$field['id'].'" id="'.$field_id.'" type="text" />';
					$html.= '<div id="'.$button_id.'" class="upload-button button hana-upload-btn" >Upload</div><br/>';
				...
				case 'select':
					...
					if(!empty($field['options'])) foreach($field['options'] as $option) {
						$html .= '<option';
						if ( $field['std'] == $option['id']) {
		...
		$html.='<input type="hidden" name="category" value="'.$cat_id.'" class="category" />';
